...
from src.command_parser import command_parser, RainbowLexer
...
def show_all(contacts, *args):
    ...
    print_list = Paginator(contacts).get_view()
...
def show_birthday(contacts, *args):
    ...
    print_list = Paginator(contacts).get_view(func=func_days)
    for item in print_list:
        result += f'{item}'
    return result
...
def search(contacts, *args):
    ...
    if len(args) == 1:
        ...
        print_list = Paginator(contacts).get_view(func=func_sub)
        for item in print_list:
            if item is None:
